...
class Fitting:
    ...
    def objective(self, theta, psi):
        ...
        obj1 = (self.T - integrate[:,7])@self.weights@(self.T - integrate[:,7])
        obj2 = (self.D - integrate[:,6])@self.weights@(self.D - integrate[:,6])
    ...
    def fit(self, psi, x0, bounds, algorithm = 'L-BFGS-B'):
        ...
        # Estimate variances
        self.sigma2_1 = (self.T - curve[:,7])@self.weights@(self.T - curve[:,7])/(n-K)
        self.sigma2_2 = (self.D - curve[:,6])@self.weights@(self.D - curve[:,6])/(n-K)  
